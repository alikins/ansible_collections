# Ansible module: ansible.module_nxos_acl_interface
...
``` json
{
    "direction": "{'description': ['Direction ACL to be applied in on the interface.'], 'required': True, 'choices': ['ingress', 'egress']}",
    "interface": "{'description': ['Full name of interface, e.g. I(Ethernet1/1).'], 'required': True}",
    ...
    "state": "{'description': ['Specify desired state of the resource.'], 'required': False, 'default': 'present', 'choices': ['present', 'absent']}",
}
```
