# Ansible module: ansible.module_ovirt_vm_facts
...
``` json
{
    "all_content": "{'description': ['If I(true) all the attributes of the virtual machines should be included in the response.'], 'type': 'bool'}",
    ...
    "case_sensitive": "{'description': ['If I(true) performed search will take case into account.'], 'type': 'bool', 'default': True}",
    ...
    "max": "{'description': ['The maximum number of results to return.']}",
    ...
    "pattern": "{'description': ['Search term which is accepted by oVirt/RHV search backend.', 'For example to search VM X from cluster Y use following pattern: name=X and cluster=Y']}",
}
```
