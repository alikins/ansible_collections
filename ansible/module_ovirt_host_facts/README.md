# Ansible module: ansible.module_ovirt_host_facts
...
``` json
{
    "all_content": "{'description': ['If I(true) all the attributes of the hosts should be included in the response.'], 'default': False, 'version_added': '2.7', 'type': 'bool'}",
    ...
    "pattern": "{'description': ['Search term which is accepted by oVirt/RHV search backend.', 'For example to search host X from datacenter Y use following pattern: name=X and datacenter=Y']}",
}
```
