# Ansible module: ansible.module_nxos_switchport
...
``` json
{
    "access_vlan": "{'description': ['If C(mode=access), used as the access VLAN ID.']}",
    "interface": "{'description': ['Full name of the interface, i.e. Ethernet1/1.']}",
    "mode": "{'description': ['Mode for the Layer 2 port.'], 'choices': ['access', 'trunk']}",
    "native_vlan": "{'description': ['If C(mode=trunk), used as the trunk native VLAN ID.']}",
    ...
    "trunk_allowed_vlans": "{'description': ['if C(mode=trunk), these are the only VLANs that will be configured on the trunk, i.e. "2-10,15".'], 'version_added': 2.2}",
    "trunk_vlans": "{'description': ['If C(mode=trunk), used as the VLAN range to ADD or REMOVE from the trunk.'], 'aliases': ['trunk_add_vlans']}",
}
```
