# Ansible module: ansible.module_ovirt_user_facts
...
``` json
{
    ...
    "pattern": "{'description': ['Search term which is accepted by oVirt/RHV search backend.', 'For example to search user X use following pattern: name=X']}",
}
```
