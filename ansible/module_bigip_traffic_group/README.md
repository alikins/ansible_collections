# Ansible module: ansible.module_bigip_traffic_group
...
``` json
{
    ...
    "state": "{'description': ['When C(present), ensures that the traffic group exists.', 'When C(absent), ensures the traffic group is removed.'], 'default': 'present', 'choices': ['present', 'absent']}",
