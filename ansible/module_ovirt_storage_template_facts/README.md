# Ansible module: ansible.module_ovirt_storage_template_facts
...
``` json
{
    ...
    "max": "{'description': ['Sets the maximum number of templates to return. If not specified all the templates are returned.']}",
    ...
    "storage_domain": "{'description': ['The storage domain name where the templates should be listed.']}",
    "unregistered": "{'description': ['Flag which indicates whether to get unregistered templates which contain one or more disks which reside on a storage domain or diskless templates.'], 'type': 'bool', 'default': False}",
}
```
