# Ansible module: ansible.module_os_ironic_inspect
...
``` json
{
    ...
    "mac": "{'description': ['unique mac address that is used to attempt to identify the host.']}",
    "name": "{'description': ['unique name identifier to identify the host in Ironic.']}",
    "region_name": "{'description': ['Name of the region.'], 'required': False}",
    "timeout": "{'description': ['A timeout in seconds to tell the role to wait for the node to complete introspection if wait is set to True.'], 'required': False, 'default': 1200}",
    "uuid": "{'description': ['globally unique identifier (UUID) to identify the host.']}",
