# Ansible module: ansible.module_bigip_firewall_policy
...
``` json
{
    ...
    "state": "{'description': ['When C(state) is C(present), ensures that the policy exists.', 'When C(state) is C(absent), ensures that the policy is removed.'], 'choices': ['present', 'absent'], 'default': 'present'}",
